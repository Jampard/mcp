...
- **Access to Docs with Pagination**: Fetch official Vendure docs with smart pagination support - get table of contents, specific sections by name, or paginate by page number to stay within token limits.
...
## Documentation Access with Pagination

The `vendure_get_docs` tool provides smart access to Vendure documentation with built-in pagination to avoid token limits:
...
{ "type": "standard" }
```
Returns a list of all available sections with examples of how to fetch specific content.

**Get Specific Section:**
```json
{ "type": "standard", "section": "Core Concepts" }
```
Fetches just the "Core Concepts" section (supports case-insensitive partial matching).

**Page-based Pagination:**
```json
{ "type": "standard", "page": 1, "pageSize": 5000 }
```
Fetches page 1 with 5000 lines per page.

### Options
- `type`: `"standard"` (overview) or `"full"` (comprehensive reference)
- `section`: Section title to retrieve (optional, case-insensitive)
- `page`: Page number for numeric pagination (optional, starts at 1)
- `pageSize`: Lines per page (optional, default: 5000)
